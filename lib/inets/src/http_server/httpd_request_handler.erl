		    ...
		    case httpd_request:body_chunk_first(Body, Length, MaxChunk) of 
			    ...
			    {noreply, State#state{mfa = 
                        ...
                        {ok, {{continue, Chunk}, Module, Function, Args}} ->
                            handle_internal_chunk(State#state{chunk =  {first, undefined}, 
                                                              body = Chunk}, Module, Function, Args);                   
                        {ok, NewBody} ->
                            handle_response(State#state{chunk = {undefined, undefined},
                                              headers = Headers,
                                              body = NewBody})
		    end;
		false ->
...
handle_response(#state{body    = Body, 
    ...
    handle_next_request(State#state{chunk = {undefined, undefined}, response_sent = true}, Data);
handle_response(#state{body    = Body, 
		       mod     = ModData, 
		       headers = Headers,
		       max_keep_alive_request = Max} = State) when Max > 0 ->
