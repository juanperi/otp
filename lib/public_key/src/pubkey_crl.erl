...
verify_crl(OtpCert, DP, CRL, DerCRL, DeltaCRL, DerDeltaCRL, OtherDPCRLs,
	...
	{ok, Issuer} ->
		...
		{ok, TrustedOtpCert, Path} ->
		    ...
		    {invalid, RevokedState#revoke_state{valid_ext = ValidExt, details = [{bad_crl, no_issuer_cert_chain} | Details]}}
            end;
	{error, issuer_not_found} ->
		...
		_ ->
		    ...
		    {invalid, {skip, State0#revoke_state{details = [{bad_crl, no_issuer_cert_chain} | Details] }}}
            end
    catch
	throw:{bad_crl, invalid_issuer} = Reason ->
            Details = RevokedState#revoke_state.details,
	    {invalid, {skip, RevokedState#revoke_state{details = [Reason | Details]}}};
	throw:Reason ->
            Details = RevokedState#revoke_state.details,
	    {invalid, RevokedState#revoke_state{details =  [Reason | Details]}}
    end.
...
verify_mask_and_signatures(Revoked, DeltaRevoked, RevokedState, CRL, DerCRL, DeltaCRL, DerDeltaCRL,
	...
	error:{badmatch, _} ->
            Details = RevokedState#revoke_state.details,
	    {invalid, RevokedState#revoke_state{details = [{bad_crl, invalid_signature}| Details]}}
    end.
