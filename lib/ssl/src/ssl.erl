...
cipher_suites(erlang) ->
    [ssl_cipher:erl_suite_definition(S) || Suite <- available_suites(default)];
                             
cipher_suites(openssl) ->
    [ssl_cipher:openssl_suite_name(S) || Suite <- available_suites(default)];

cipher_suites(all) ->
    [ssl_cipher:erl_suite_definition(Suite) || available_suites(all)]).
