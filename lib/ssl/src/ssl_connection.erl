...
handle_call({shutdown, How0}, From, StateName,
	...
	How when How == write; How == both ->	    
            transport_shutdown_alert(StateName, State);
	_ ->
	    ok
    end,
...
%%% Internal functions
%%--------------------------------------------------------------------
send_alert(#alert{level = ?FATAL} = Alert, connection, #state{protocol_cb = Connection} = State) ->
    Connection:send_sync_alert(Alert, State);
...
transport_shutdown_alert(connection, #state{protocol_cb = Connection} = State) ->
