...
init_per_suite(Config) ->
    ...
    try (crypto:start() == ok) andalso 
             ssl_test_lib:sufficient_crypto_support('tlsv1.2') 
    of
	true ->
            ssl:stop(),
            ...
            [{default_client_conf, ClientConf}, {default_server_conf, ServerConf} | Config]
    catch _:_ ->
...
verify_peer(Config) when is_list(Config) ->
    ClientOpts = proplists:get_value(default_client_conf),
    ServerOpts = proplists:get_value(default_server_conf),
    ssl_certificate_verify:verify_peer(ClientOpts, 
...
verify_none(Config) when is_list(Config) ->
    ClientOpts = proplists:get_value(default_client_conf),
    ServerOpts = proplists:get_value(default_server_conf),
    ssl_certificate_verify:verify_none(ClientOpts, 
                                       {ssl_test_lib, send_recv_result_active, []},
                                       ServerOpts, 
                                       {ssl_test_lib, send_recv_result_active, []},
                                       active).
...
server_verify_client_once(Config) when is_list(Config) ->
    ClientOpts = proplists:get_value(default_client_conf),
    ServerOpts = proplists:get_value(default_server_conf),
    ssl_certificate_verify:verify_client_once(ClientOpts, 
                                              {ssl_test_lib, send_recv_result_active, []},
                                              ServerOpts, 
                                              {ssl_test_lib, send_recv_result_active, []},
                                              active).
...
server_require_peer_cert_ok(Config) when is_list(Config) ->
    ClientOpts = proplists:get_value(default_client_conf),
    ServerOpts0 = proplists:get_value(default_server_conf),
    ServerOpts = [{fail_if_no_peer_cert, true} | ServerOpts0],
    ssl_certificate_verify:verify_peer(ClientOpts, 
                                       {ssl_test_lib, send_recv_result_active, []},
                                       ServerOpts, 
                                       {ssl_test_lib, send_recv_result_active, []},
